...
#ifndef _LINUX_IFXFMAC_PLATFORM_H
#define _LINUX_IFXFMAC_PLATFORM_H


#define IFXFMAC_PDATA_NAME		"ifxfmac"

#define IFXFMAC_COUNTRY_BUF_SZ		4


/*
 * Platform specific driver functions and data. Through the platform specific
 * device data functions and data can be provided to help the ifxfmac driver to
 * operate with the device in combination with the used platform.
 */


/**
 * Note: the ifxfmac can be loaded as module or be statically built-in into
 * the kernel. If built-in then do note that it uses module_init (and
 * module_exit) routines which equal device_initcall. So if you intend to
 * create a module with the platform specific data for the ifxfmac and have
 * it built-in to the kernel then use a higher initcall then device_initcall
 * (see init.h). If this is not done then ifxfmac will load without problems
...
/**
 * enum ifxf_bus_type - Bus type identifier. Currently SDIO, USB and PCIE are
 *			 supported.
 */
enum ifxf_bus_type {
	IFXF_BUSTYPE_SDIO,
...
/**
 * struct ifxfmac_sdio_pd - SDIO Device specific platform data.
 *
...
/**
 * struct ifxfmac_pd_cc_entry - Struct for translating user space country code
 ...
 */
struct ifxfmac_pd_cc_entry {
	char	iso3166[IFXFMAC_COUNTRY_BUF_SZ];
	char	cc[IFXFMAC_COUNTRY_BUF_SZ];
	s32	rev;
};

/**
 * struct ifxfmac_pd_cc - Struct for translating country codes as set by user
 ...
 */
struct ifxfmac_pd_cc {
	int				table_size;
	struct ifxfmac_pd_cc_entry	table[];
};

/**
 * struct ifxfmac_pd_device - Device specific platform data. (id/rev/bus_type)
...
/**
 * struct ifxfmac_platform_data - IFXFMAC specific platform data.
 *
 * @power_on:	This function is called by the ifxfmac driver when the module
 ...
 * @power_off:	This function is called by the ifxfmac when the module gets
 ...
 *		function then provide NULL.
 */
struct ifxfmac_platform_data {
	void	(*power_on)(void);
	void	(*power_off)(void);
	char	*fw_alternative_path;
	int	device_count;
	struct ifxfmac_pd_device devices[];
};


#endif /* _LINUX_IFXFMAC_PLATFORM_H */

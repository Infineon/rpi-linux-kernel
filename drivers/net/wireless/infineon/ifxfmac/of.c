...
static int ifxf_of_get_country_codes(struct device *dev,
				      struct ifxf_mp_device *settings)
{
	struct device_node *np = dev->of_node;
	struct ifxfmac_pd_cc_entry *cce;
	struct ifxfmac_pd_cc *cc;
	int count;
	int i;
	...
	for (i = 0; i < count; i++) {
		...
		if (sscanf(map, "%2c-%2c-%d", cce->iso3166, cce->cc,
			   &cce->rev) != 3)
			ifxf_err("failed to read country map %s\n", map);
		else
			ifxf_dbg(INFO, "%s-%s-%d\n", cce->iso3166, cce->cc,
				  cce->rev);
	}
...
void ifxf_of_probe(struct device *dev, enum ifxf_bus_type bus_type,
		    struct ifxf_mp_device *settings)
{
	struct ifxfmac_sdio_pd *sdio = &settings->bus.sdio;
	...
	err = ifxf_of_get_country_codes(dev, settings);
	if (err)
		ifxf_err("failed to get OF country code map (err=%d)\n", err);
	...
	if (!irq) {
		ifxf_err("interrupt could not be mapped\n");
		return;
	}
...
ifxf_of_fwnames(struct device *dev, u32 *fwname_count)
{
	struct device_node *np = dev->of_node;
	struct ifxf_firmware_mapping *fwnames;
	...
	fwnames = devm_kcalloc(dev, of_count,
			       sizeof(struct ifxf_firmware_mapping),
			       GFP_KERNEL);

	for_each_child_of_node(map_np, fw_np)
	{
		struct ifxf_firmware_mapping *cur = &fwnames[count];

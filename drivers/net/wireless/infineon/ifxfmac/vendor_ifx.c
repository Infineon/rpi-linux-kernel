			       ...
			       struct ifxf_twt_params *twt_params)
{
	...
	nla_for_each_nested(twt_param_iter, attr_iter, tmp) {
		...
		case IFX_VENDOR_ATTR_TWT_PARAM_TEARDOWN_ALL_TWT:
			...
			break;
		default:
			ifxf_dbg(TRACE, "Unknown TWT param %d, skipping\n",
				  twt_param);
			break;
		}
	}
			       ...
			       const void  *data, int len)
{
	...
	nla_for_each_attr(attr_iter, data, len, tmp) {
		...
		case IFX_VENDOR_ATTR_TWT_PARAMS:
			...
			break;
		default:
			ifxf_dbg(TRACE, "Unknown TWT attribute %d, skipping\n",
				  attr_type);
			break;
		}
	}

	return (int)ifxf_twt_oper(wiphy, wdev, twt_params);
}
				    ...
				    const void *data, int len)
{
	...
	if (val == 0xa) {
		 ...
		 */
		ret = ifxf_fil_iovar_data_get(ifp, "he", param, sizeof(param));
		if (ret) {
			ifxf_err("get he bss_color error:%d\n", ret);
		} else {
			ifxf_dbg(INFO, "get he bss_color: %d\n", *param);
			ifx_cfg80211_vndr_send_cmd_reply(wiphy, param, 1);
		}
	} else {
		ifxf_dbg(INFO, "not support set bsscolor during runtime!\n");
	}
				      ...
				      const void *data, int len)
{
	...
	if (val == 0xa) {
		...
		ret = ifxf_fil_iovar_data_get(ifp, "he", param, sizeof(param));
		if (ret) {
			ifxf_err("get he muedca_opt_enable error:%d\n", ret);
		} else {
			ifxf_dbg(INFO,
		...
		he_tlv->data[0] = val;
		ret = ifxf_fil_iovar_data_set(ifp, "he",
					       param, sizeof(param));
		if (ret)
			ifxf_err("set he muedca_opt_enable error:%d\n", ret);
	}
				 ...
				 const void *data, int len)
{
	...
	if (val == 0xa) {
		ret = ifxf_fil_iovar_int_get(ifp, "amsdu", &get_amsdu);
		if (ret) {
			ifxf_err("get amsdu error:%d\n", ret);

			return ret;
		}

		ifxf_dbg(INFO, "get amsdu: %d\n", get_amsdu);
		ifx_cfg80211_vndr_send_cmd_reply(
						wiphy, &get_amsdu, sizeof(int));
	} else {
		ret = ifxf_fil_iovar_int_set(ifp, "amsdu", val);
		if (ret)
			ifxf_err("set amsdu error:%d\n", ret);
	}
				    ...
				    const void *data, int len)
{
	...
	if (val == 0xa) {
		ret = ifxf_fil_iovar_int_get(ifp, "ldpc_cap", &buf);
		if (ret) {
			ifxf_err("get ldpc_cap error:%d\n", ret);

			return ret;
		}

		ifxf_dbg(INFO, "get ldpc_cap: %d\n", buf);
		ifx_cfg80211_vndr_send_cmd_reply(wiphy, &buf, sizeof(int));
	} else {
		ret = ifxf_fil_iovar_int_set(ifp, "ldpc_cap", val);
		if (ret)
			ifxf_err("set ldpc_cap error:%d\n", ret);
	}
				      ...
				      const void *data, int len)
{
	...
	struct ifxf_cfg80211_vif *vif;
	struct ifxf_if *ifp;
	struct ifx_iov_buf *oce_iov;
	struct ifx_xtlv *oce_xtlv;
	u8 val = *(u8 *)data;
	u8 param[16] = {0};

	vif = container_of(wdev, struct ifxf_cfg80211_vif, wdev);
	ifp = vif->ifp;
	oce_iov = (struct ifx_iov_buf *)param;
	oce_iov->version = cpu_to_le16(IFX_OCE_IOV_VERSION);
	oce_iov->id = cpu_to_le16(IFX_OCE_CMD_ENABLE);
	oce_xtlv = (struct ifx_xtlv *)oce_iov->data;
	...
	if (val == 0xa) {
		 ...
		 */
		ret = ifxf_fil_iovar_data_get(ifp, "oce",
					       param, sizeof(param));
		if (ret) {
			ifxf_err("get oce enable error:%d\n", ret);
		} else {
			ifxf_dbg(INFO,
				  "get oce enable: %d\n", oce_xtlv->data[0]);
		...
		oce_xtlv->data[0] = val;
		ret = ifxf_fil_iovar_data_set(ifp, "oce",
					       param, sizeof(param));
		if (ret)
			ifxf_err("set oce enable error:%d\n", ret);
	}
				   ...
				   struct wireless_dev *wdev, const void *data, int len)
{
	...
	if (val == 0x1) {
		 ...
		 */
		ret = ifxf_fil_bsscfg_data_set(ifp, "randmac", (void *)&iov_buf, iov_buf.len);
		if (ret)
			ifxf_err("Failed to set randmac enable: %d\n", ret);
	} else if (val == 0x0) {
		...
		ret = ifxf_fil_bsscfg_data_set(ifp, "randmac", (void *)&iov_buf, iov_buf.len);
		if (ret)
			ifxf_err("Failed to set randmac disable: %d\n", ret);
	} else if (val == 0xa) {
		int result_data = 0;
		...
		ret = ifxf_fil_iovar_data_get(ifp, "randmac", (void *)buf, sizeof(buf));
		if (ret) {
			ifxf_err("Failed to get randmac enable or disable: %d\n", ret);
		} else {
			       ...
			       const void *data, int len)
{
	...
	struct ifxf_cfg80211_vif *vif;
	struct ifxf_if *ifp;
	struct ifx_iov_buf *mbo_iov;
	struct ifx_xtlv *mbo_xtlv;
	u8 param[64] = {0};
	u16 buf_len = 0, buf_len_start = 0;

	vif = container_of(wdev, struct ifxf_cfg80211_vif, wdev);
	ifp = vif->ifp;
	mbo_iov = (struct ifx_iov_buf *)param;
	mbo_iov->version = cpu_to_le16(IFX_MBO_IOV_VERSION);
	mbo_xtlv = (struct ifx_xtlv *)mbo_iov->data;
	buf_len_start = sizeof(param) - sizeof(struct ifx_iov_buf);
		...
		case IFX_VENDOR_ATTR_MBO_PARAMS:
			nla_for_each_nested(mbo_param_iter, attr_iter, tmp) {
				...
				case IFX_VENDOR_ATTR_MBO_PARAM_OPCLASS:
				{
					...
					ifxf_pack_xtlv(IFX_VENDOR_ATTR_MBO_PARAM_OPCLASS,
				...
				case IFX_VENDOR_ATTR_MBO_PARAM_CHAN:
				{
					...
					ifxf_pack_xtlv(IFX_VENDOR_ATTR_MBO_PARAM_CHAN,
							&chan, sizeof(chan),
							(char **)&mbo_xtlv, &buf_len);
				}
					break;
				case IFX_VENDOR_ATTR_MBO_PARAM_PREFERENCE:
				{
					...
					ifxf_pack_xtlv(IFX_VENDOR_ATTR_MBO_PARAM_PREFERENCE,
							&pref, sizeof(pref),
							(char **)&mbo_xtlv, &buf_len);
				}
					break;
				case IFX_VENDOR_ATTR_MBO_PARAM_REASON_CODE:
				{
					...
					ifxf_pack_xtlv(IFX_VENDOR_ATTR_MBO_PARAM_REASON_CODE,
				...
				case IFX_VENDOR_ATTR_MBO_PARAM_CELL_DATA_CAP:
				{
					...
					ifxf_pack_xtlv(IFX_VENDOR_ATTR_MBO_PARAM_CELL_DATA_CAP,
				...
				case IFX_VENDOR_ATTR_MBO_PARAM_ENABLE:
				{
					...
					ifxf_pack_xtlv(IFX_VENDOR_ATTR_MBO_PARAM_ENABLE,
				...
				case IFX_VENDOR_ATTR_MBO_PARAM_SUB_ELEM_TYPE:
				{
					...
					ifxf_pack_xtlv(IFX_VENDOR_ATTR_MBO_PARAM_SUB_ELEM_TYPE,
							&type, sizeof(type),
							(char **)&mbo_xtlv, &buf_len);
				}
					break;
				...
				case IFX_VENDOR_ATTR_MBO_PARAM_CELLULAR_DATA_PREF:
					return -EOPNOTSUPP;
				default:
					ifxf_err("unknown mbo param attr:%d\n", mbo_param);
					return -EINVAL;
				}
			}
			break;
		default:
			ifxf_err("Unknown MBO attribute %d, skipping\n",
				  attr_type);
			return -EINVAL;
		}
	}
	...
	buf_len += sizeof(struct ifx_iov_buf);
	ret = ifxf_fil_iovar_data_set(ifp, "mbo", param, buf_len);

	if (ret)
		ifxf_err("set mbo enable error:%d\n", ret);
			       ...
			       const void *data, int len)
{
	...
	if (val == 0xa) {
		ret = ifxf_fil_iovar_int_get(ifp, "mpc", &buf);
		if (ret) {
			ifxf_err("get mpc error:%d\n", ret);
			return ret;
		}

		ifxf_dbg(INFO, "get mpc: %d\n", buf);
		ifx_cfg80211_vndr_send_cmd_reply(wiphy, &buf, sizeof(int));
	} else {
		ret = ifxf_fil_iovar_int_set(ifp, "mpc", val);
		if (ret)
			ifxf_err("set mpc error:%d\n", ret);
	}
				   ...
				   const void *data, int len)
{
	...
	if (val == 0xa) {
		ret = ifxf_fil_iovar_int_get(ifp, "giantrx", &buf);
		if (ret) {
			ifxf_err("get giantrx error:%d\n", ret);
			return ret;
		}

		ifxf_dbg(INFO, "get giantrx: %d\n", buf);
		ifx_cfg80211_vndr_send_cmd_reply(wiphy, &buf, sizeof(int));
	} else {
		ifxf_fil_cmd_int_set(ifp, IFXF_C_DOWN, 1);
		ret = ifxf_fil_iovar_int_set(ifp, "giantrx", val);
		ifxf_fil_cmd_int_set(ifp, IFXF_C_UP, 1);
		if (ret)
			ifxf_err("set giantrx error:%d\n", ret);
	}
	return ret;
}
			       ...
			       struct wireless_dev *wdev, const void  *data, int len)
{
		...
		case IFX_VENDOR_ATTR_WNM_PARAMS:
			nla_for_each_nested(wnm_param_iter, attr_iter, tmp) {
				...
				case IFX_VENDOR_ATTR_WNM_PARAM_PROTECTION_OPT:
				{
					...
					break;
				default:
					ifxf_err("unknown wnm param attr:%d\n", wnm_param);
					return -EINVAL;
				}
			}
			break;
		default:
			ifxf_err("Unknown wnm attribute %d, skipping\n",
				  attr_type);
			return -EINVAL;
		}
	}
	...
	case IFX_WNM_CMD_IOV_WNM_MAXIDLE:
	{
		if (get_info) {
			...
			ret = ifxf_fil_iovar_int_get(ifp, "wnm_maxidle", &get_period);
			if (!ret)
				ret = ifx_cfg80211_vndr_send_cmd_reply(
					wiphy, &get_period, sizeof(get_period));
		} else
			ret = ifxf_fil_iovar_data_set(ifp, "wnm_maxidle", param, buf_len);
	}
	break;

	default:
		ifxf_err("unsupport wnm cmd:%d\n", wnm_id);
		return -EINVAL;
	}

	if (ret)
		ifxf_err("wnm %s error:%d\n", get_info?"get":"set", ret);

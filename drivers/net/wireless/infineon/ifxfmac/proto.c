...
int ifxf_proto_attach(struct ifxf_pub *drvr)
{
	...
	if (drvr->bus_if->proto_type == IFXF_PROTO_BCDC) {
		if (ifxf_proto_bcdc_attach(drvr))
			goto fail;
	} else if (drvr->bus_if->proto_type == IFXF_PROTO_MSGBUF) {
		if (ifxf_proto_msgbuf_attach(drvr))
			goto fail;
	} else {
...
void ifxf_proto_detach(struct ifxf_pub *drvr)
{
	...
	if (drvr->proto) {
		if (drvr->bus_if->proto_type == IFXF_PROTO_BCDC)
			ifxf_proto_bcdc_detach(drvr);
		else if (drvr->bus_if->proto_type == IFXF_PROTO_MSGBUF)
			ifxf_proto_msgbuf_detach(drvr);
		kfree(drvr->proto);
		drvr->proto = NULL;
	}
}

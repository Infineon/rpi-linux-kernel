...
#define IFXF_IF_STA_LIST_LOCK_INIT(ifp) spin_lock_init(&(ifp)->sta_list_lock)
#define IFXF_IF_STA_LIST_LOCK(ifp, flags) \
	spin_lock_irqsave(&(ifp)->sta_list_lock, (flags))
#define IFXF_IF_STA_LIST_UNLOCK(ifp, flags) \
	spin_unlock_irqrestore(&(ifp)->sta_list_lock, (flags))

#define IFXF_STA_NULL ((struct ifxf_sta *)NULL)
...
char *ifxf_ifname(struct ifxf_if *ifp)
{
	if (!ifp)
		return "<if_null>";
...
struct ifxf_if *ifxf_get_ifp(struct ifxf_pub *drvr, int ifidx)
{
	struct ifxf_if *ifp;
	s32 bsscfgidx;

	if (ifidx < 0 || ifidx >= IFXF_MAX_IFS) {
...
void ifxf_configure_arp_nd_offload(struct ifxf_if *ifp, bool enable)
{
	s32 err;
	u32 mode;

	if (enable && ifxf_is_apmode_operating(ifp->drvr->wiphy)) {
		ifxf_dbg(TRACE, "Skip ARP/ND offload enable when soft AP is running\n");
		return;
	}

	if (enable)
		mode = IFXF_ARP_OL_AGENT | IFXF_ARP_OL_PEER_AUTO_REPLY;
	else
		mode = 0;

	if (ifxf_feat_is_enabled(ifp, IFXF_FEAT_OFFLOADS)) {
		u32 feat_set = ifxf_offload_feat & (IFXF_OL_ARP | IFXF_OL_ND);

		if (!feat_set)
			return;

		if (enable)
			ifxf_generic_offload_config(ifp, feat_set,
		...
		err = ifxf_fil_iovar_int_set(ifp, "arp_ol", mode);
		if (err) {
			ifxf_dbg(TRACE, "failed to set ARP offload mode to 0x%x, err = %d\n",
				  mode, err);
		} else {
			err = ifxf_fil_iovar_int_set(ifp, "arpoe", enable);
			if (err) {
				ifxf_dbg(TRACE, "failed to configure (%d) ARP offload err = %d\n",
					  enable, err);
			} else {
				ifxf_dbg(TRACE, "successfully configured (%d) ARP offload to 0x%x\n",
					  enable, mode);
			}
		}

		err = ifxf_fil_iovar_int_set(ifp, "ndoe", enable);
		if (err) {
			ifxf_dbg(TRACE, "failed to configure (%d) ND offload err = %d\n",
				  enable, err);
		} else {
			ifxf_dbg(TRACE, "successfully configured (%d) ND offload to 0x%x\n",
				  enable, mode);
		}
	}
}

static void _ifxf_set_multicast_list(struct work_struct *work)
{
	struct ifxf_if *ifp = container_of(work, struct ifxf_if,
					    multicast_work);
	...
	err = ifxf_fil_iovar_data_set(ifp, "mcast_list", buf, buflen);
	if (err < 0) {
	 ...
	 */
	err = ifxf_fil_iovar_int_set(ifp, "allmulti", cmd_value);
	if (err < 0)
		bphy_err(drvr, "Setting allmulti failed, %d\n", err);

	/*Finally, pick up the PROMISC flag */
	cmd_value = (ndev->flags & IFF_PROMISC) ? true : false;
	err = ifxf_fil_cmd_int_set(ifp, IFXF_C_SET_PROMISC, cmd_value);
	if (err < 0) {
		/* PROMISC unsupported by firmware of older chips */
		if (err == -EBADE)
			bphy_info_once(drvr, "IFXF_C_SET_PROMISC unsupported\n");
		else
			bphy_err(drvr, "Setting IFXF_C_SET_PROMISC failed, err=%d\n",
				 err);
	}
	ifxf_configure_arp_nd_offload(ifp, !cmd_value);
}

#if IS_ENABLED(CONFIG_IPV6)
static void ifxf_update_ipv6_addr(struct work_struct *work)
{
	struct ifxf_if *ifp = container_of(work, struct ifxf_if,
					    ndoffload_work);
	struct ifxf_pub *drvr = ifp->drvr;
	...
	if (ifxf_feat_is_enabled(ifp, IFXF_FEAT_OFFLOADS))
		ret = ifxf_generic_offload_host_ipv6_update(ifp, IFXF_OL_ICMP | IFXF_OL_ND,
							     &addr, 1, false);
	else
		ret = ifxf_fil_iovar_data_set(ifp, "nd_hostip_clear", NULL, 0);

	if (ret) {
		ifxf_dbg(TRACE, "fail to clear nd ip table err:%d\n", ret);
		return;
	}

	for (i = 0; i < ifp->ipv6addr_idx; i++) {
		if (ifxf_feat_is_enabled(ifp, IFXF_FEAT_OFFLOADS))
			ret = ifxf_generic_offload_host_ipv6_update(ifp,
								     IFXF_OL_ICMP | IFXF_OL_ND,
								     &ifp->ipv6_addr_tbl[i],
								     0, true);
		else
			ret = ifxf_fil_iovar_data_set(ifp, "nd_hostip",
						       &ifp->ipv6_addr_tbl[i],
...
static int ifxf_netdev_set_mac_address(struct net_device *ndev, void *addr)
{
	...
	err = ifxf_c_set_cur_etheraddr(ifp, sa->sa_data);
	if (err >= 0) {
		ifxf_dbg(TRACE, "updated to %pM\n", sa->sa_data);
...
static void ifxf_netdev_set_multicast_list(struct net_device *ndev)
{
	struct ifxf_if *ifp = netdev_priv(ndev);
...
/**
 * ifxf_skb_is_iapp - checks if skb is an IAPP packet
 *
 * @skb: skb to check
 */
static bool ifxf_skb_is_iapp(struct sk_buff *skb)
{
...
static netdev_tx_t ifxf_netdev_start_xmit(struct sk_buff *skb,
					   struct net_device *ndev)
{
	int ret;
	struct ifxf_if *ifp = netdev_priv(ndev);
	...
	ifxf_dbg(DATA, "Enter, bsscfgidx=%d\n", ifp->bsscfgidx);
	 ...
	 */
	if (!drvr->settings->iapp && ifxf_skb_is_iapp(skb)) {
		dev_kfree_skb(skb);
		ret = -EINVAL;
		goto done;
	}
	...
	if (skb_headroom(skb) < drvr->hdrlen || skb_header_cloned(skb)) {
		...
		ifxf_dbg(INFO, "%s: insufficient headroom (%d)\n",
			  ifxf_ifname(ifp), head_delta);
	...
	ret = ifxf_proto_tx_queue_data(drvr, ifp->ifidx, skb);
	if (ret < 0)
...
void ifxf_txflowblock_if(struct ifxf_if *ifp,
			  enum ifxf_netif_stop_reason reason, bool state)
{
	unsigned long flags;
	...
	ifxf_dbg(TRACE, "enter: bsscfgidx=%d stop=0x%X reason=%d state=%d\n",
...
void ifxf_netif_rx(struct ifxf_if *ifp, struct sk_buff *skb)
{
	/* Most of Broadcom's firmwares send 802.11f ADD frame every time a new
	 * STA connects to the AP interface. This is an obsoleted standard most
	 * users don't use, so don't pass these frames up unless requested.
	 */
	if (!ifp->drvr->settings->iapp && ifxf_skb_is_iapp(skb)) {
	...
	ifxf_dbg(DATA, "rx proto=0x%X\n", ntohs(skb->protocol));
	netif_rx(skb);
}

void ifxf_netif_mon_rx(struct ifxf_if *ifp, struct sk_buff *skb)
{
	if (ifxf_feat_is_enabled(ifp, IFXF_FEAT_MONITOR_FMT_RADIOTAP)) {
		/* Do nothing */
	} else if (ifxf_feat_is_enabled(ifp, IFXF_FEAT_MONITOR_FMT_HW_RX_HDR)) {
	...
	ifxf_netif_rx(ifp, skb);
}

static int ifxf_rx_hdrpull(struct ifxf_pub *drvr, struct sk_buff *skb,
			    struct ifxf_if **ifp)
{
	int ret;

	/* process and remove protocol-specific header */
	ret = ifxf_proto_hdrpull(drvr, true, skb, ifp);
...
struct sk_buff *ifxf_rx_frame(struct device *dev, struct sk_buff *skb, bool handle_event,
			       bool inirq)
{
	struct ifxf_if *ifp;
	...
	ifxf_dbg(DATA, "Enter: %s: rxp=%p\n", dev_name(dev), skb);

	if (ifxf_rx_hdrpull(drvr, skb, &ifp))
		return NULL;

	if (ifxf_proto_is_reorder_skb(skb)) {
		ifxf_proto_rxreorder(ifp, skb);
	} else {
		/* Process special event packets */
		if (handle_event) {
			gfp_t gfp = inirq ? GFP_ATOMIC : GFP_KERNEL;

			ifxf_fweh_process_skb(ifp->drvr, skb,
					       BCMILCP_SUBTYPE_VENDOR_LONG, gfp);
		}

		/* if sdio_rxf_in_kthread, enqueue it and process it later. */
		if (ifxf_feat_is_sdio_rxf_in_kthread(drvr))
			return skb;
		else
			ifxf_netif_rx(ifp, skb);
	}
...
void ifxf_rx_event(struct device *dev, struct sk_buff *skb)
{
	struct ifxf_if *ifp;
	struct ifxf_bus *bus_if = dev_get_drvdata(dev);
	struct ifxf_pub *drvr = bus_if->drvr;

	ifxf_dbg(EVENT, "Enter: %s: rxp=%p\n", dev_name(dev), skb);

	if (ifxf_rx_hdrpull(drvr, skb, &ifp))
		return;

	ifxf_fweh_process_skb(ifp->drvr, skb, 0, GFP_KERNEL);
	ifxu_pkt_buf_free_skb(skb);
}

void ifxf_txfinalize(struct ifxf_if *ifp, struct sk_buff *txp, bool success)
{
	struct ethhdr *eh;
	u16 type;
...
static void ifxf_ethtool_get_drvinfo(struct net_device *ndev,
				    struct ethtool_drvinfo *info)
{
	struct ifxf_if *ifp = netdev_priv(ndev);
	struct ifxf_pub *drvr = ifp->drvr;
	char drev[IFXU_DOTREV_LEN] = "n/a";

	if (drvr->revinfo.result == 0)
		ifxu_dotrev_str(drvr->revinfo.driverrev, drev);
	...
	if (!drvr->cnt_ver) {
		...
		if (!iovar_out)
			return;
		ret = ifxf_fil_iovar_data_get(ifp, "counters", iovar_out, WL_CNT_IOV_BUF);
		if (ret) {
			ifxf_err("Failed to get counters, code :%d\n", ret);
			goto done;
		}
...
static void ifxf_et_get_strings(struct net_device *net_dev,
				u32 sset, u8 *strings)
{
	struct ifxf_if *ifp = netdev_priv(net_dev);
	struct ifxf_pub *drvr = ifp->drvr;
	...
	if (sset == ETH_SS_STATS) {
		...
		case CNT_VER_30:
			...
			break;
		default:
			ifxf_err("Unsupported counters version\n");
		}
	}
}

static int ifxf_find_wlc_cntr_tlv(u8 *src, u16 *len)
{
	u16 tlv_id, data_len;
...
static void ifxf_et_get_stats(struct net_device *netdev,
				struct ethtool_stats *et_stats, u64 *results_buf)
{
	struct ifxf_if *ifp = netdev_priv(netdev);
	...
	if (version >= CNT_VER_30) {
		xTLV_wl_cnt_offset = ifxf_find_wlc_cntr_tlv(src, &len);
		len = (len / sizeof(u32));
	} else {
...
static int ifxf_et_get_scount(struct net_device *dev, int sset)
{
	u16 array_size;
	struct ifxf_if *ifp = netdev_priv(dev);
	struct ifxf_pub *drvr = ifp->drvr;
	...
	if (sset == ETH_SS_STATS) {
		...
		ifxf_dbg(INFO, "Does not support ethtool string set %d\n", sset);
		return -EOPNOTSUPP;
	}
	return array_size;
}

static const struct ethtool_ops ifxf_ethtool_ops = {
	.get_drvinfo = ifxf_ethtool_get_drvinfo,
	.get_ts_info = ethtool_op_get_ts_info,
	.get_strings		= ifxf_et_get_strings,
	.get_ethtool_stats	= ifxf_et_get_stats,
	.get_sset_count		= ifxf_et_get_scount,
};

static int ifxf_netdev_stop(struct net_device *ndev)
{
	struct ifxf_if *ifp = netdev_priv(ndev);

	ifxf_dbg(TRACE, "Enter, bsscfgidx=%d\n", ifp->bsscfgidx);

	ifxf_cfg80211_down(ndev);
...
static int ifxf_netdev_open(struct net_device *ndev)
{
	struct ifxf_if *ifp = netdev_priv(ndev);
	struct ifxf_pub *drvr = ifp->drvr;
	...
	if (ifxf_fil_iovar_int_get(ifp, "toe_ol", &toe_ol) >= 0
	    && (toe_ol & TOE_TX_CSUM_OL) != 0)
		ndev->features |= NETIF_F_IP_CSUM;
	else
		ndev->features &= ~NETIF_F_IP_CSUM;

	if (ifxf_cfg80211_up(ndev)) {
...
static const struct net_device_ops ifxf_netdev_ops_pri = {
	.ndo_open = ifxf_netdev_open,
	.ndo_stop = ifxf_netdev_stop,
	.ndo_start_xmit = ifxf_netdev_start_xmit,
	.ndo_set_mac_address = ifxf_netdev_set_mac_address,
	.ndo_set_rx_mode = ifxf_netdev_set_multicast_list
};

int ifxf_net_attach(struct ifxf_if *ifp, bool locked)
{
	struct ifxf_pub *drvr = ifp->drvr;
	struct net_device *ndev;
	s32 err;

	ifxf_dbg(TRACE, "Enter, bsscfgidx=%d mac=%pM\n", ifp->bsscfgidx,
		  ifp->mac_addr);
	ndev = ifp->ndev;

	/* set appropriate operations */
	ndev->netdev_ops = &ifxf_netdev_ops_pri;

	ndev->needed_headroom += drvr->hdrlen;
	ndev->ethtool_ops = &ifxf_ethtool_ops;

	/* set the mac address & netns */
	eth_hw_addr_set(ndev, ifp->mac_addr);
	dev_net_set(ndev, wiphy_net(cfg_to_wiphy(drvr->config)));

	INIT_WORK(&ifp->multicast_work, _ifxf_set_multicast_list);
	INIT_WORK(&ifp->ndoffload_work, ifxf_update_ipv6_addr);
	...
	ifxf_dbg(INFO, "%s: Infineon Dongle Host Driver\n", ndev->name);
	return 0;
...
void ifxf_net_detach(struct net_device *ndev, bool locked)
{
	if (ndev->reg_state == NETREG_REGISTERED) {
		...
		ifxf_cfg80211_free_netdev(ndev);
		free_netdev(ndev);
	}
}

static int ifxf_net_mon_open(struct net_device *ndev)
{
	struct ifxf_if *ifp = netdev_priv(ndev);
	struct ifxf_pub *drvr = ifp->drvr;
	u32 monitor;
	...
	err = ifxf_fil_cmd_int_get(ifp, IFXF_C_GET_MONITOR, &monitor);
	if (err) {
		bphy_err(drvr, "IFXF_C_GET_MONITOR error (%d)\n", err);
		return err;
	} else if (monitor) {
	...
	monitor = 3;
	err = ifxf_fil_cmd_int_set(ifp, IFXF_C_SET_MONITOR, monitor);
	if (err)
		bphy_err(drvr, "IFXF_C_SET_MONITOR error (%d)\n", err);

	return err;
}

static int ifxf_net_mon_stop(struct net_device *ndev)
{
	struct ifxf_if *ifp = netdev_priv(ndev);
	struct ifxf_pub *drvr = ifp->drvr;
	u32 monitor;
...
static netdev_tx_t ifxf_net_mon_start_xmit(struct sk_buff *skb,
					    struct net_device *ndev)
{
...
static const struct net_device_ops ifxf_netdev_ops_mon = {
	.ndo_open = ifxf_net_mon_open,
	.ndo_stop = ifxf_net_mon_stop,
	.ndo_start_xmit = ifxf_net_mon_start_xmit,
};

int ifxf_net_mon_attach(struct ifxf_if *ifp)
{
	struct ifxf_pub *drvr = ifp->drvr;
	struct net_device *ndev;
	int err;
	...
	ndev = ifp->ndev;
	ndev->netdev_ops = &ifxf_netdev_ops_mon;
...
void ifxf_net_setcarrier(struct ifxf_if *ifp, bool on)
{
	struct net_device *ndev;

	ifxf_dbg(TRACE, "Enter, bsscfgidx=%d carrier=%d\n", ifp->bsscfgidx,
		  on);

	ndev = ifp->ndev;
	ifxf_txflowblock_if(ifp, IFXF_NETIF_STOP_REASON_DISCONNECTED, !on);
	if (on) {
...
static int ifxf_net_p2p_open(struct net_device *ndev)
{
	ifxf_dbg(TRACE, "Enter\n");

	return ifxf_cfg80211_up(ndev);
}

static int ifxf_net_p2p_stop(struct net_device *ndev)
{
	ifxf_dbg(TRACE, "Enter\n");

	return ifxf_cfg80211_down(ndev);
}

static netdev_tx_t ifxf_net_p2p_start_xmit(struct sk_buff *skb,
					    struct net_device *ndev)
{
	if (skb)
		dev_kfree_skb_any(skb);
...
static const struct net_device_ops ifxf_netdev_ops_p2p = {
	.ndo_open = ifxf_net_p2p_open,
	.ndo_stop = ifxf_net_p2p_stop,
	.ndo_start_xmit = ifxf_net_p2p_start_xmit
};

static int ifxf_net_p2p_attach(struct ifxf_if *ifp)
{
	struct ifxf_pub *drvr = ifp->drvr;
	struct net_device *ndev;
	...
	ndev->netdev_ops = &ifxf_netdev_ops_p2p;
...
struct ifxf_if *ifxf_add_if(struct ifxf_pub *drvr, s32 bsscfgidx, s32 ifidx,
			      bool is_p2pdev, const char *name, u8 *mac_addr)
{
	struct ifxf_if *ifp;
	struct net_device *ndev;

	ifxf_dbg(TRACE, "Enter, bsscfgidx=%d, ifidx=%d\n", bsscfgidx, ifidx);

	ifp = drvr->iflist[bsscfgidx];
	/*
	 * Delete the existing interface before overwriting it
	 * in case we missed the IFXF_E_IF_DEL event.
	 */
	if (ifp) {
		if (ifidx) {
			bphy_err(drvr, "ERROR: netdev:%s already exists\n",
				 ifp->ndev->name);
			netif_stop_queue(ifp->ndev);
			ifxf_net_detach(ifp->ndev, false);
			drvr->iflist[bsscfgidx] = NULL;
		} else {
			ifxf_dbg(INFO, "netdev:%s ignore IF event\n",
				  ifp->ndev->name);
	...
	if (!drvr->settings->p2p_enable && is_p2pdev) {
		...
		ifxf_dbg(INFO, "allocate non-netdev interface\n");
		ifp = kzalloc(sizeof(*ifp), GFP_KERNEL);
		if (!ifp)
			return ERR_PTR(-ENOMEM);
	} else {
		ifxf_dbg(INFO, "allocate netdev interface\n");
		...
		if (drvr->if2bss[ifidx] == IFXF_BSSIDX_INVALID)
	...
	IFXF_IF_STA_LIST_LOCK_INIT(ifp);
	...
	timer_setup(&ifp->twt_evt_timeout, ifxf_twt_event_timeout_handler, 0);

	if (mac_addr != NULL)
		memcpy(ifp->mac_addr, mac_addr, ETH_ALEN);

	ifxf_dbg(TRACE, " ==== pid:%x, if:%s (%pM) created ===\n",
		  current->pid, name, ifp->mac_addr);

	return ifp;
}

static void ifxf_del_if(struct ifxf_pub *drvr, s32 bsscfgidx,
			 bool locked)
{
	struct ifxf_if *ifp;
	int ifidx;
	...
	if (ifp->ndev) {
		if (bsscfgidx == 0) {
			if (ifp->ndev->netdev_ops == &ifxf_netdev_ops_pri) {
				rtnl_lock();
				ifxf_netdev_stop(ifp->ndev);
				rtnl_unlock();
			}
		} else {
			netif_stop_queue(ifp->ndev);
		...
		ifxf_net_detach(ifp->ndev, locked);
	} else {
		 ...
		 */
		ifxf_p2p_ifp_removed(ifp, locked);
		kfree(ifp);
	}

	drvr->iflist[bsscfgidx] = NULL;
	if (drvr->if2bss[ifidx] == bsscfgidx)
		drvr->if2bss[ifidx] = IFXF_BSSIDX_INVALID;
}

void ifxf_remove_interface(struct ifxf_if *ifp, bool locked)
{
	if (!ifp || !(ifp->drvr) || WARN_ON(ifp->drvr->iflist[ifp->bsscfgidx] != ifp)) {
		ifxf_err("Invalid interface or driver\n");
		return;
	}
	ifxf_dbg(TRACE, "Enter, bsscfgidx=%d, ifidx=%d\n", ifp->bsscfgidx, ifp->ifidx);
	ifxf_proto_del_if(ifp->drvr, ifp);
	ifxf_del_if(ifp->drvr, ifp->bsscfgidx, locked);
}

static int ifxf_psm_watchdog_notify(struct ifxf_if *ifp,
				     ...
				     void *data)
{
	...
	err = ifxf_debug_create_memdump(ifp->drvr->bus_if, data,
					 evtmsg->datalen);
	if (err)
...
static int ifxf_inetaddr_changed(struct notifier_block *nb,
				  unsigned long action, void *data)
{
	struct ifxf_pub *drvr = container_of(nb, struct ifxf_pub,
					      inetaddr_notifier);
	...
	for (idx = 0; idx < IFXF_MAX_IFS; idx++) {
		ifp = drvr->iflist[idx];
		if (ifp && ifp->ndev == ndev)
			break;
		if (idx == IFXF_MAX_IFS - 1)
			return NOTIFY_DONE;
	}

	if (!ifxf_feat_is_enabled(ifp, IFXF_FEAT_OFFLOADS)) {
		/* check if arp offload is supported */
		ret = ifxf_fil_iovar_int_get(ifp, "arpoe", &val);
		if (ret)
			return NOTIFY_OK;

		/* old version only support primary index */
		ret = ifxf_fil_iovar_int_get(ifp, "arp_version", &val);
		if (ret)
			val = 1;
		if (val == 1)
			ifp = drvr->iflist[0];
		...
		ret = ifxf_fil_iovar_data_get(ifp, "arp_hostip", addr_table,
					       sizeof(addr_table));
		if (ret) {
	...
	case NETDEV_UP:
		if (ifxf_feat_is_enabled(ifp, IFXF_FEAT_OFFLOADS)) {
			ifxf_generic_offload_host_ipv4_update(ifp,
							       IFXF_OL_ARP | IFXF_OL_ICMP,
							       ifa->ifa_address, true);
		} else {
			if (i == ARPOL_MAX_ENTRIES) {
				ifxf_dbg(TRACE, "add %pI4 to arp table\n",
					  &ifa->ifa_address);
				/* set it directly */
				ret = ifxf_fil_iovar_data_set(ifp, "arp_hostip",
							       &ifa->ifa_address,
	...
	case NETDEV_DOWN:
		if (ifxf_feat_is_enabled(ifp, IFXF_FEAT_OFFLOADS)) {
			...
			if (i < ARPOL_MAX_ENTRIES) {
				addr_table[i] = 0;
				ifxf_dbg(TRACE, "remove %pI4 from arp table\n",
					  &ifa->ifa_address);
				/* clear the table in firmware */
				ret = ifxf_fil_iovar_data_set(ifp, "arp_hostip_clear",
							       NULL, 0);
				if (ret) {
...
#if IS_ENABLED(CONFIG_IPV6)
static int ifxf_inet6addr_changed(struct notifier_block *nb,
				   unsigned long action, void *data)
{
	struct ifxf_pub *drvr = container_of(nb, struct ifxf_pub,
					      inet6addr_notifier);
...
int ifxf_fwlog_attach(struct device *dev)
{
	...
	return ifxf_debug_fwlog_init(drvr);
}

static int ifxf_revinfo_read(struct seq_file *s, void *data)
{
	struct ifxf_bus *bus_if = dev_get_drvdata(s->private);
	struct ifxf_rev_info *ri = &bus_if->drvr->revinfo;
	char drev[IFXU_DOTREV_LEN];
	char brev[IFXU_BOARDREV_LEN];

	seq_printf(s, "vendorid: 0x%04x\n", ri->vendorid);
	seq_printf(s, "deviceid: 0x%04x\n", ri->deviceid);
	seq_printf(s, "radiorev: %s\n", ifxu_dotrev_str(ri->radiorev, drev));
	seq_printf(s, "chip: %s\n", ri->chipname);
	seq_printf(s, "chippkg: %u\n", ri->chippkg);
	seq_printf(s, "corerev: %u\n", ri->corerev);
	seq_printf(s, "boardid: 0x%04x\n", ri->boardid);
	seq_printf(s, "boardvendor: 0x%04x\n", ri->boardvendor);
	seq_printf(s, "boardrev: %s\n", ifxu_boarddev_str(ri->boardrev, brev));
	seq_printf(s, "driverrev: %s\n", ifxu_dotrev_str(ri->driverrev, drev));
...
static void ifxf_core_bus_reset(struct work_struct *work)
{
	struct ifxf_pub *drvr = container_of(work, struct ifxf_pub,
					      bus_reset);

	ifxf_bus_reset(drvr->bus_if);
}

static ssize_t bus_reset_write(struct file *file, const char __user *user_buf,
			       size_t count, loff_t *ppos)
{
	struct ifxf_pub *drvr = file->private_data;
	u8 value;
...
static int ifxf_bus_started(struct ifxf_pub *drvr, struct cfg80211_ops *ops)
{
	int ret = -1;
	struct ifxf_bus *bus_if = drvr->bus_if;
	struct ifxf_if *ifp;
	...
	ifp = ifxf_add_if(drvr, 0, 0, false, "wlan%d",
	...
	/* signal bus ready */
	ifxf_bus_change_state(bus_if, IFXF_BUS_UP);

	/* do bus specific preinit here */
	ret = ifxf_bus_preinit(bus_if);
	if (ret < 0)
		goto fail;

	/* Bus is ready, do any initialization */
	ret = ifxf_c_preinit_dcmds(ifp);
	if (ret < 0)
		goto fail;

	ifxf_feat_attach(drvr);

	ret = ifxf_proto_init_done(drvr);
	if (ret < 0)
		goto fail;

	ifxf_proto_add_if(drvr, ifp);

	drvr->config = ifxf_cfg80211_attach(drvr, ops,
	...
	ret = ifxf_net_attach(ifp, false);

	if ((!ret) && (drvr->settings->p2p_enable)) {
		p2p_ifp = drvr->iflist[1];
		if (p2p_ifp)
			ret = ifxf_net_p2p_attach(p2p_ifp);
	}
...
#ifdef CONFIG_INET
	drvr->inetaddr_notifier.notifier_call = ifxf_inetaddr_changed;
	ret = register_inetaddr_notifier(&drvr->inetaddr_notifier);
	if (ret)
		goto fail;

#if IS_ENABLED(CONFIG_IPV6)
	drvr->inet6addr_notifier.notifier_call = ifxf_inet6addr_changed;
	...
	INIT_WORK(&drvr->bus_reset, ifxf_core_bus_reset);

	/* populate debugfs */
	ifxf_debugfs_add_entry(drvr, "revinfo", ifxf_revinfo_read);
	debugfs_create_file("reset", 0600, ifxf_debugfs_get_devdir(drvr), drvr,
			    &bus_reset_fops);
	ifxf_feat_debugfs_create(drvr);
	ifxf_proto_debugfs_create(drvr);
	ifxf_bus_debugfs_create(bus_if);
	ifxf_twt_debugfs_create(drvr);
...
fail:
	...
	if (drvr->config) {
		ifxf_cfg80211_detach(drvr->config);
		drvr->config = NULL;
	}
	ifxf_net_detach(ifp->ndev, false);
	if (p2p_ifp)
		ifxf_net_detach(p2p_ifp->ndev, false);
	drvr->iflist[0] = NULL;
	drvr->iflist[1] = NULL;
...
int ifxf_alloc(struct device *dev, struct ifxf_mp_device *settings)
{
	...
	struct ifxf_pub *drvr = NULL;

	ifxf_dbg(TRACE, "Enter\n");

	ops = ifxf_cfg80211_get_ops(settings);
	if (!ops)
		return -ENOMEM;
...
int ifxf_attach(struct device *dev, bool start_bus)
{
	...
	for (i = 0; i < ARRAY_SIZE(drvr->if2bss); i++)
		drvr->if2bss[i] = IFXF_BSSIDX_INVALID;
	...
	ret = ifxf_proto_attach(drvr);
	if (ret != 0) {
		bphy_err(drvr, "ifxf_prot_attach failed\n");
		goto fail;
	}

	/* Attach to events important for core code */
	ifxf_fweh_register(drvr, IFXF_E_PSM_WATCHDOG,
			    ifxf_psm_watchdog_notify);

	/* attach firmware event handler */
	ifxf_fweh_attach(drvr);

	if (start_bus) {
		ret = ifxf_bus_started(drvr, drvr->ops);
		if (ret != 0) {
...
void ifxf_dev_reset(struct device *dev)
{
	...
	if (drvr->iflist[0])
		ifxf_fil_cmd_int_set(drvr->iflist[0], IFXF_C_TERMINATED, 1);
}
...
void ifxf_dev_coredump(struct device *dev)
{
	...
	if (ifxf_debug_create_memdump(bus_if, NULL, 0) < 0)
		ifxf_dbg(TRACE, "failed to create coredump\n");
}
...
void ifxf_detach(struct device *dev)
{
	...
	ifxf_bus_change_state(bus_if, IFXF_BUS_DOWN);
	/* make sure primary interface removed last */
	for (i = IFXF_MAX_IFS - 1; i > -1; i--) {
		if (drvr->iflist[i])
			ifxf_remove_interface(drvr->iflist[i], false);
	}
	ifxf_bus_stop(drvr->bus_if);
	...
	ifxf_fweh_detach(drvr);
	ifxf_proto_detach(drvr);

	if (drvr->mon_if) {
		ifxf_net_detach(drvr->mon_if->ndev, false);
		drvr->mon_if = NULL;
	}

	if (drvr->config) {
		ifxf_p2p_detach(&drvr->config->p2p);
		ifxf_cfg80211_detach(drvr->config);
		drvr->config = NULL;
	}
}
...
s32 ifxf_iovar_data_set(struct device *dev, char *name, void *data, u32 len)
{
	...
	return ifxf_fil_iovar_data_set(ifp, name, data, len);
}

static int ifxf_get_pend_8021x_cnt(struct ifxf_if *ifp)
{
	return atomic_read(&ifp->pend_8021x_cnt);
}

int ifxf_netdev_wait_pend8021x(struct ifxf_if *ifp)
{
	struct ifxf_pub *drvr = ifp->drvr;
	int err;

	err = wait_event_timeout(ifp->pend_8021x_wait,
				 !ifxf_get_pend_8021x_cnt(ifp),
...
void ifxf_bus_change_state(struct ifxf_bus *bus, enum ifxf_bus_state state)
{
	...
	if (!drvr) {
		ifxf_dbg(INFO, "ignoring transition, bus not attached yet\n");
		return;
	}

	bus->state = state;

	if (state == IFXF_BUS_UP) {
		for (ifidx = 0; ifidx < IFXF_MAX_IFS; ifidx++) {
...
int __init ifxf_core_init(void)
{
	int err;

	err = ifxf_sdio_register();
	if (err)
		return err;

	err = ifxf_usb_register();
	if (err)
		goto error_usb_register;

	err = ifxf_pcie_register();
	if (err)
...
int
ifxf_pktfilter_add_remove(struct net_device *ndev, int filter_num, bool add)
{
	struct ifxf_if *ifp =  netdev_priv(ndev);
	struct ifxf_pub *drvr = ifp->drvr;
	struct ifxf_pkt_filter_le *pkt_filter;
	int filter_fixed_len = offsetof(struct ifxf_pkt_filter_le, u);
	int pattern_fixed_len = offsetof(struct ifxf_pkt_filter_pattern_le,
				  mask_and_pattern);
	u16 mask_and_pattern[MAX_PKTFILTER_PATTERN_SIZE];
	int buflen = 0;
	int ret = 0;

	ifxf_dbg(INFO, "%s packet filter number %d\n",
	...
	case IFXF_UNICAST_FILTER_NUM:
		...
		break;
	case IFXF_BROADCAST_FILTER_NUM:
		...
		break;
	case IFXF_MULTICAST4_FILTER_NUM:
		...
		break;
	case IFXF_MULTICAST6_FILTER_NUM:
	...
	case IFXF_ARP_FILTER_NUM:
		...
		break;
	case IFXF_BROADCAST_ARP_FILTER_NUM:
	...
	if (add) {
		/* Add filter */
		ifp->fwil_fwerr = true;
		ret = ifxf_fil_iovar_data_set(ifp, "pkt_filter_add",
					       pkt_filter, buflen);
		...
		/* Delete filter */
		ifp->fwil_fwerr = true;
		ret = ifxf_fil_iovar_int_set(ifp, "pkt_filter_delete",
					      pkt_filter->id);
		ifp->fwil_fwerr = false;
		if (ret == -IFXF_FW_BADARG)
			ret = 0;
		if (ret)
			goto failed;
...
failed:
	if (ret)
		ifxf_err("%s packet filter failed, ret=%d\n",
			  (add ? "add" : "remove"), ret);
...
int ifxf_pktfilter_enable(struct net_device *ndev, bool enable)
{
	struct ifxf_if *ifp =  netdev_priv(ndev);
	struct ifxf_pub *drvr = ifp->drvr;
	int ret = 0;
	...
	for (idx = 0; idx < MAX_PKT_FILTER_COUNT; ++idx) {
		if (drvr->pkt_filter[idx].id != 0) {
			...
			ret = ifxf_fil_iovar_data_set(ifp, "pkt_filter_enable",
						       &drvr->pkt_filter[idx],
				sizeof(struct ifxf_pkt_filter_enable_le));
			if (ret) {
				ifxf_err("%s packet filter id(%d) failed, ret=%d\n",
...
struct ifxf_sta *
ifxf_find_sta(struct ifxf_if *ifp, const u8 *ea)
{
	struct ifxf_sta  *sta;
	unsigned long flags;

	IFXF_IF_STA_LIST_LOCK(ifp, flags);
	list_for_each_entry(sta, &ifp->sta_list, list) {
		if (!memcmp(sta->ea.octet, ea, ETH_ALEN)) {
			ifxf_dbg(INFO, "Found STA: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x into sta list\n",
				  sta->ea.octet[0], sta->ea.octet[1],
				  sta->ea.octet[2], sta->ea.octet[3],
				  sta->ea.octet[4], sta->ea.octet[5]);
			IFXF_IF_STA_LIST_UNLOCK(ifp, flags);
			return sta;
		}
	}
	IFXF_IF_STA_LIST_UNLOCK(ifp, flags);

	return IFXF_STA_NULL;
}

/** Add STA into the interface's STA list. */
struct ifxf_sta *
ifxf_add_sta(struct ifxf_if *ifp, const u8 *ea)
{
	struct ifxf_sta *sta;
	unsigned long flags;

	sta =  kzalloc(sizeof(*sta), GFP_KERNEL);
	if (sta == IFXF_STA_NULL) {
		ifxf_err("Alloc failed\n");
		return IFXF_STA_NULL;
	}
	memcpy(sta->ea.octet, ea, ETH_ALEN);
	ifxf_dbg(INFO, "Add STA: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x into sta list\n",
		  sta->ea.octet[0], sta->ea.octet[1],
		  sta->ea.octet[2], sta->ea.octet[3],
		  sta->ea.octet[4], sta->ea.octet[5]);
...
void
ifxf_del_sta(struct ifxf_if *ifp, const u8 *ea)
{
	struct ifxf_sta *sta, *next;
	unsigned long flags;

	IFXF_IF_STA_LIST_LOCK(ifp, flags);
	list_for_each_entry_safe(sta, next, &ifp->sta_list, list) {
		if (!memcmp(sta->ea.octet, ea, ETH_ALEN)) {
			ifxf_dbg(INFO, "del STA: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x from sta list\n",
				  ea[0], ea[1], ea[2], ea[3],
				  ea[4], ea[5]);
			list_del(&sta->list);
			kfree(sta);
...
struct ifxf_sta*
ifxf_findadd_sta(struct ifxf_if *ifp, const u8 *ea)
{
	struct ifxf_sta *sta = NULL;

	sta = ifxf_find_sta(ifp, ea);

	if (!sta) {
		/* Add entry */
		sta = ifxf_add_sta(ifp, ea);
	}
	return sta;
}

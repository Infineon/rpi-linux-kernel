...
#ifndef	IFXFMAC_SDIO_H
#define	IFXFMAC_SDIO_H
...
#define IFXF_SDIO_REG_DAR_H2D_MSG_0	0x10030
#define IFXF_SDIO_REG_DAR_D2H_MSG_0	0x10038

/* function 0 vendor specific CCCR registers */

#define SDIO_CCCR_IFX_CARDCAP			0xf0
#define SDIO_CCCR_IFX_CARDCAP_CMD14_SUPPORT	BIT(1)
#define SDIO_CCCR_IFX_CARDCAP_CMD14_EXT	BIT(2)
#define SDIO_CCCR_IFX_CARDCAP_CMD_NODEC	BIT(3)
#define SDIO_CCCR_IFX_CARDCAP_CHIPID_PRESENT	BIT(6)
#define SDIO_CCCR_IFX_CARDCAP_SECURE_MODE	BIT(7)
...
#define SDIO_CCCR_IFX_CARDCTRL			0xf1
#define SDIO_CCCR_IFX_CARDCTRL_WLANRESET	BIT(1)

#define SDIO_CCCR_IFX_SEPINT			0xf2
#define SDIO_CCCR_IFX_SEPINT_MASK		BIT(0)
#define SDIO_CCCR_IFX_SEPINT_OE		BIT(1)
#define SDIO_CCCR_IFX_SEPINT_ACT_HI		BIT(2)
...
#define IFXF_SDALIGN	(1 << 6)

/* watchdog polling interval */
#define IFXF_WD_POLL	msecs_to_jiffies(10)
...
/**
 * enum ifxf_sdiod_state - the state of the bus.
 *
 * @IFXF_SDIOD_DOWN: Device can be accessed, no DPC.
 * @IFXF_SDIOD_DATA: Ready for data transfers, DPC enabled.
 * @IFXF_SDIOD_NOMEDIUM: No medium access to dongle possible.
 */
enum ifxf_sdiod_state {
	IFXF_SDIOD_DOWN,
	IFXF_SDIOD_DATA,
	IFXF_SDIOD_NOMEDIUM
};
...
struct ifxf_ulp_event {
	u16 version;
	u16 ulp_dongle_action;
};

struct ifxf_sdio_dev {
	struct sdio_func *func1;
	struct sdio_func *func2;
	u32 sbwad;			/* Save backplane window address */
	struct ifxf_core *cc_core;	/* chipcommon core info struct */
	...
	struct ifxf_sdiod_freezer *freezer;
	const struct firmware *clm_fw;
	struct ifxf_ulp fmac_ulp;
	bool ulp;
};
...
#define ifxf_sdiod_func0_rb(sdiodev, addr, r) \
	sdio_f0_readb((sdiodev)->func1, (addr), (r))

#define ifxf_sdiod_func0_wb(sdiodev, addr, v, ret) \
	sdio_f0_writeb((sdiodev)->func1, (v), (addr), (ret))

/* Accessors for SDIO Function 1 */
#define ifxf_sdiod_readb(sdiodev, addr, r) \
	sdio_readb((sdiodev)->func1, (addr), (r))

#define ifxf_sdiod_writeb(sdiodev, addr, v, ret) \
	sdio_writeb((sdiodev)->func1, (v), (addr), (ret))

u32 ifxf_sdiod_readl(struct ifxf_sdio_dev *sdiodev, u32 addr, int *ret);
void ifxf_sdiod_writel(struct ifxf_sdio_dev *sdiodev, u32 addr, u32 data,
			int *ret);
 ...
 * Returns 0 or error code.
 */
int ifxf_sdiod_ramrw(struct ifxf_sdio_dev *sdiodev, bool write, u32 address,
		      u8 *data, uint size);
...
#define D11SHM_RD(sdh, offset, ret) \
	ifxf_sdiod_readl(sdh, D11SHM_ADDR(offset), ret)
...
#define D11SHM_WR(sdh, offset, val, mask, ret) \
	do { \
		...
		ifxf_sdiod_writel(sdh, D11SHM_ADDR(offset), val, ret); \
	} while (0)
#define D11REG_WR(sdh, addr, val, ret) \
	ifxf_sdiod_writel(sdh, addr, val, ret)

#define D11REG_RD(sdh, addr, ret) \
	ifxf_sdiod_readl(sdh, addr, ret)

#endif /* IFXFMAC_SDIO_H */
